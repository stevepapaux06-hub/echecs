...
export function ChessPathApp() {
  ...
  if (screen === "training-hub") return <main className="app-page"><AppNav active="training-hub" {...navProps} /><TrainingHub exercises={hubExercises} priority={result?.metrics.priorityTitle ?? persistent?.analyses[0]?.metrics.priorityTitle} onStart={(items) => void startTraining(items)} onAnalyze={() => navigate("analyze")} /></main>;
