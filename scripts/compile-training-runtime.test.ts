...
import { runtimeBankFingerprint } from "../src/domain/training/runtime-bank-fingerprint";
...
it("compiles the exact active bank for the browser", () => {
  ...
  const fingerprint = runtimeBankFingerprint(exercises);
  ...
  writeFileSync("src/domain/training/runtime-bank.generated.ts", `// Generated; do not edit.\nexport const RUNTIME_BANK_MANIFEST = ${JSON.stringify({ count: exercises.length, fingerprint })} as const;\nexport default JSON.parse(${JSON.stringify(serialized)}) as unknown[];\n`);
  console.info(`Runtime bank: ${exercises.length} exercises, ${serialized.length} bytes, ${fingerprint}`);
}, 300_000);
